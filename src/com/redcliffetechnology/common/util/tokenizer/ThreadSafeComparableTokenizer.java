...
public class ThreadSafeComparableTokenizer<T> implements Tokenizer<T> {
	...
	private final ConcurrentHashMap<T, VolatileComparableTokenized<T>> readTokenSpace;
	private final NavigableMap<T, VolatileComparableTokenized<T>> tokenSpace;
	private int tokenSpaceVersion;

	public ThreadSafeComparableTokenizer(Comparator<? super T> comparator) {
		this(comparator,false);
	}

	public ThreadSafeComparableTokenizer(Comparator<? super T> comparator, boolean retainAll) {
		tokenSpaceVersion=0;
		tokenSpace=(comparator==null)?new TreeMap<T,VolatileComparableTokenized<T>>():new TreeMap<T,VolatileComparableTokenized<T>>(comparator);
		//tokenSpace=(comparator==null)?new ConcurrentSkipListMap<T,Tokenized<T>>():new ConcurrentSkipListMap<T,Tokenized<T>>(comparator);
		readTokenSpace=new ConcurrentHashMap<T,VolatileComparableTokenized<T>>();
		this.retainAll=retainAll;
	}

	public ThreadSafeComparableTokenizer() {
		this(null,false);
	}

	public ThreadSafeComparableTokenizer(boolean retainAll) {
		this(null,false);
	}
	...
	public VolatileComparableTokenized<T> getTokenized(T instance) {
		VolatileComparableTokenized<T> stub=readTokenSpace.get(instance);
		if(stub==null) {
	...
	private final void redistribute() {
		...
		//T oldObject=null;
		for(VolatileComparableTokenized<T> stub: tokenSpace.values()) {
	...
	private final synchronized VolatileComparableTokenized<T> tokenize(T instance) {
		...
		final Map.Entry<T, VolatileComparableTokenized<T>> beforeEntry=tokenSpace.floorEntry(instance);
		final VolatileComparableTokenized<T> before=(beforeEntry==null)?null:beforeEntry.getValue();
		if(before!=null && compare(before.getObject(),instance)==0) {
			// Same
			if(retainAll) {
				final VolatileComparableTokenized<T> stub=new VolatileComparableTokenized<T>(instance, before);
		...
		final Map.Entry<T, VolatileComparableTokenized<T>> afterEntry=tokenSpace.ceilingEntry(instance);
		final VolatileComparableTokenized<T> after=(afterEntry==null)?null:afterEntry.getValue();
		if(after!=null && compare(after.getObject(),instance)==0) {
			// Same
			if(retainAll) {
				final VolatileComparableTokenized<T> stub=new VolatileComparableTokenized<T>(instance, after);
				readTokenSpace.put(instance, stub);
				return stub;
			}
			else {
		...
		final VolatileComparableTokenized<T> stub=new VolatileComparableTokenized<T>(instance,value+ADD,tokenSpaceVersion);
